synchronized 方法是针对同一个对象的同一个方法的修饰符，确保一个方法在一个时间点只有一个线程操作，
...
为什么我要提毫秒数呢？因为加了synchronized方法之后，给人的感觉是“一个线程”在操作，消耗时间和一个线程卖500张票是差不多的，其实，不是，因为线程操作完sell()之后便会

继续执行代码，这里只有一个sell(),所以看不出来，我们用sleep方法，模拟其余代码执行的时间。
...
**但是去掉sleep方法，单线程运行时间比多线程低，单线20ms，多线33ms**这个。。。值得思考
